...
#rx_invalid = re.compile("^192\.168")         #toto som zakomentoval
#rx_invalid2 = re.compile("^10\.")            #toto som zakomentoval
...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def sendResponse(self,code):
        ...
        self.socket.sendto(text.encode("utf-8"),self.client_address) #socket posiela bytes takže treba text encodnuť
        showtime()
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar: #registrar.has_key(fromm): prerobene na fromm in registrar
                del registrar[fromm]
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or origin not in registrar: #not registrar.has_key(origin): - prerobene na origin not in registrar
            self.sendResponse("400 Zla poziadavka")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("destination %s" % destination)
            if destination in registrar and self.checkValidity(destination): # prerobene na if destination in registrat
    ...
    def processAck(self):
        ...
        if len(destination) > 0:
            logging.info("destination %s" % destination)
            if destination in registrar:  #registrar.has_key(destination): - prerobene
                socket,claddr = self.getSocketInfo(destination)
                #self.changeRequestUri()
                self.data = self.addTopVia()
                data = self.removeRouteHeader()
                ...
                data.insert(1,recordroute)
                text = "\r\n".join(data) #text = string.join(data,"\r\n") - prerobene 
                socket.sendto(text.encode("utf-8"),claddr) #+ encode textu na bytes
                showtime()
                logging.info("<<< %s" % data[0])
                logging.debug( "---\n<< server send [%d]:\n%s\n---" % (len(text),text))
    ...
    def processNonInvite(self):
        ...
        logging.debug("----------------------")
        origin = self.getOrigin()
        if len(origin) == 0 or origin not in registrar: #not registrar.has_key(origin): prerobene
            self.sendResponse("400 Zla poziadavka")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info("destination %s" % destination)
            if destination in registrar and self.checkValidity(destination): #prerobene
                socket,claddr = self.getSocketInfo(destination)
                #self.changeRequestUri()
                self.data = self.addTopVia()
                data = self.removeRouteHeader()
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:  #registrar.has_key(origin): prerobene
